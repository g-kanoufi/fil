...
export function FddBulkSendModal({ open, leadIds, onClose, onComplete }: FddBulkSendModalProps) {
  ...
  return (
    <ModalDialog
      ...
      className="max-w-md shadow-[var(--shadow-card)]"
    >
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <Button type="button" disabled={sendingType !== null} onClick={() => void onSend('unit')}>
...
export function FddBulkSendBar({ selectedCount, onSend, onClear }: FddBulkSendBarProps) {
  ...
  return (
    <div
      className="mb-4 flex flex-wrap items-center gap-3 rounded-xl border border-accent-soft-border bg-accent-soft px-4 py-3"
      role="status"
      aria-live="polite"
      aria-atomic="true"
    >
